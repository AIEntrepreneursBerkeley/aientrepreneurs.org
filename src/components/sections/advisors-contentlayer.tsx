...
export default async function AdvisorsContentlayerSection() {
  // Fetch all members and filter advisors
  const allMembers = await fetchAllMembers();
  const advisors = allMembers.filter((member) =>
    member.role?.some((role) => role.includes("Advisor")),
  );
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl dark:border-gray-800 dark:bg-gray-900">
        <div className="p-8 text-center">
          ...
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
            Add advisors to your Notion database with "Advisor" in the role
            field.
          </p>
          <div className="mt-6">
            <Link
          ...
          <p className="mt-4 text-xs text-gray-400 dark:text-gray-500">
            Contentlayer syncs automatically on build
          </p>
        </div>
      </div>
    );
